...
public class codechef_easy_TLG1 {
	public static void main(String args[]) throws NumberFormatException, IOException{
		...
		while(rounds-->0){
			...
			sum1+=Integer.parseInt(strarray[0]);;
			sum2+=Integer.parseInt(strarray[1]);
			array[size++]=(sum1-sum2);
			}
		
		int lead=Math.abs(array[0]);
		int loc=0;
		for(int i=0;i<array.length;i++)
			if(Math.abs(array[i])>lead){
				lead=Math.abs(array[i]); 
				loc=i;
			}

		if(array[loc]>0) System.out.println(1+" "+Math.abs(array[loc]));
		else System.out.println(2+" "+Math.abs(array[loc]));
	}
}